...
read(nio::Source& in, string_view str) {
  ...
  const bool ret = n == buf.size() && buf == str;
  if (not ret) {
    in.seek(safe<i64>(pos));
  }
  return ret;
}
...
template<>
struct TracingConsumerImpl<DataType::Bool, bool> {
  u64
...
template<typename I>
struct TracingConsumerImpl<DataType::Integer, I> {
  u64
...
template<typename T>
struct TracingConsumerImpl<DataType::String, T> {
  u64
...
private:
  ...
  consumeElem(u64& result, const Elem& elem, nio::StringSink& out, Args&&... args) {
    ...
    result += TracingConsumerImpl<ElemDataType, Elem>().consume(elem, out, std::forward<Args>(args)...);
    return result;
  }
};

template<typename T>
struct TracingConsumerImpl<DataType::List, T> {
  using Elem = typename T::value_type;
  static constexpr auto ElemDataType = DataTypes<Elem>::Value;
  ...
  consume(const T& val, nio::StringSink& out) {
    ...
    auto ret = out.println("consuming list: {}", size);
    for (const auto& elem : val) {
      ret += TracingConsumerImpl<ElemDataType, Elem>().consume(elem, out);
    }
    return ret;
  }
};
...
template<>
struct TracingProducerImpl<DataType::Bool, bool> {
  void
...
template<typename I>
struct TracingProducerImpl<DataType::Integer, I> {
  void
...
template<typename T>
struct TracingProducerImpl<DataType::List, T> {
  using Elem = typename T::value_type;
  static constexpr auto ElemDataType = DataTypes<Elem>::Value;

  void
  produce(T& val, nio::Source& in, nio::Sink& out) {
    const u64 size = readI32(in);
    if constexpr (IsArray<T>) {
      out.println("producing list.array");
      for (u64 i = 0; i < size; ++i) {
        TracingProducerImpl<ElemDataType, Elem>().produce(val[i], in, out);
      }
    } else {
      out.println("producing list.containerWithPushBack");
      for (u64 i = 0; i < size; ++i) {
        val.push_back(Elem());
        TracingProducerImpl<ElemDataType, Elem>().produce(val.back(), in, out);
      }
    }
  }
};
...
TEST(codec, TracingConsumerListArray) {
  const Encoder<TracingConsumer> encoder;
  nio::StringSink out;
...
TEST(codec, TracingConsumerListVector) {
  const Encoder<TracingConsumer> encoder;
  nio::StringSink out;
...
TEST(codec, TracingConsumerOptionalAndVectorInTypeLoop) {
  ...
  {
    nio::StringSink out;
    const type val = vector<optional<i32>> { optional<i32>(1), nullopt, optional<i32>(3) };
    encoder.encode(val, out);
    EXPECT_EQ(out.str(),
...
TEST(codec, TracingProducerListArray) {
...
TEST(codec, TracingProducerListVector) {
