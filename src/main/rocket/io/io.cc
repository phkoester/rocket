    ...
    if (c == delimiter) {
      ...
      if (input.size() < min) {
        throw ParseFailure(is, pos, { inputPos, inputPos + min },
            fmt::format("Expected at least {} before {:?}, got {}", text::character(min), delimiter, input.size()));
      }
      return input;
    }
    input.push_back(c);
...
getUntil(
  ...
  while (true) {
    ...
    if (delimiter(c)) {
      ...
      if (input.size() < min) {
        throw ParseFailure(is, pos, { inputPos, inputPos + min },
            fmt::format("Expected at least {} before {}, got {}", text::character(min), delimiterDescription, input.size())); // XXX
      }
      return input;
    }
    input.push_back(c);
  ...
  while (true) {
    ...
    if (is.eof()) {
      if (input.size() >= min) {
        ...
        throw ParseFailure(is, pos, { inputPos, inputPos + min },
            fmt::format("Expected at least {} contained in {}, got {} and EOF", text::character(min), values, input.size())); // XXX
      }
    }
    check(is);
    ...
    if (values.contains(c)) {
      ...
      if (input.size() >= min) {
        ...
        throw ParseFailure(is, pos, { inputPos, inputPos + min },
            fmt::format("Expected at least {} contained in {}, got {} and {}", text::character(min), values, input.size(), c)); // XXX
      }
    }
  }
}
