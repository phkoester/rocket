...
/**
 * Makes a nonconst `unsigned char` span for the value @p v of arbitrary type @p T.
 *
...
chars(T& v) {
  return std::span<unsigned char>(reinterpret_cast<unsigned char*>(&v), sizeof(T));
}

/**
 * Makes a const `unsigned char` span for the value @p v of arbitrary type @p T.
 *
 * @tparam T the type of @p v
 * @param v an arbitrary value
 * @return a new span which serves as a byte view on @p v
 */
template<typename T>
inline std::span<const unsigned char>
chars(const T& v) {
  return std::span<const unsigned char>(reinterpret_cast<const unsigned char*>(&v), sizeof(T));
}
